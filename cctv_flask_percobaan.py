...
def count_obj(box,w,h,id):
        ...
        if int(box[1]+(box[3]-box[1])/2) > (h-1000)  and int(box[0]+(box[2]-box[0])/2)<(w-1100) and int(box[0]+(box[2]-box[0])/2)>(w-1600):
            if  id not in data:
                count += 1
                data.append(id)
    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if show_vid:
                global count
                color=(0,255,0)
                start_point = (1100, h-1000)
                end_point = (1650, h-1000)
                cv2.line(im0, start_point, end_point, color, thickness=4)
                thickness = 4
                org = (100, 100)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str, default='rtsp://admin:admin123@192.168.3.15:554/live1s1.sdp', help='source')  # file/folder, 0 for webcam
