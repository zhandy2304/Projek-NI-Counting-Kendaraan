...
def count_obj(box,w,h,id):
    ...
    if int(box[1]+(box[3]-box[1])/2) > (h-263) and int(box[1]+(box[3]-box[1])/2) < (h-235) and int(box[0]+(box[2]-box[0])/2)<(410) and int(box[0]+(box[2]-box[0])/2)>(265):
        if  id not in data:
            count += 1
            data.append(id)
...
def inputdata(car, truck):
    ...
    sql = "INSERT INTO on_rame_pettarani(waktu_input, Mobil, Bus_Truk) VALUES (now(), '"+car+"', '"+truck+"')";
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str, default='rtsp://admin:admin123@192.168.3.15:554/live1s2.sdp', help='source')  # file/folder, 0 for webcam
    ...
    parser.add_argument('--max-det', type=int, default=5000, help='maximum detection per image')
